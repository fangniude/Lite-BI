...
/**
 * 雪花算法分布式唯一ID生成器<br> 每个机器号最高支持每秒‭65535个序列, 当秒序列不足时启用备份机器号, 若备份机器也不足时借用备份机器下一秒可用序列<br> 53 bits 趋势自增ID结构如下:
 * <p>
 * |00000000|00011111|11111111|11111111|11111111|11111111|11111111|11111111| |-----------|##########32bit
 * 秒级时间戳##########|-----|-----------------| |--------------------------------------5bit机器位|xxxxx|-----------------|
...
public class Snowflake53 {
    ...
    public static long nextId() {
        return nextId(SystemClock.now() / MILLIS_IN_SECOND);
    }
    ...
    private static long nextIdBackup(long timestamp) {
        if (timestamp < lastTimestampBak) {
            if (lastTimestampBak - (SystemClock.now() / MILLIS_IN_SECOND) <= BACK_TIME_MAX) {
